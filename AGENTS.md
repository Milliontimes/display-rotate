...
- **托盘命令路径：已实机验收（2026-09-18）**。不靠点鼠标 —— 直接向托盘的消息窗口（`FindWindowW("DisplayRotateTrayClass","DisplayRotateTray")`，实测 hwnd 属于托盘 PID）发消息，走的是**同一条真实命令路径**（WndProc → `TrayCommand` → 主循环 → `rotate::set_orientation`），每步之后用独立的 `EnumDisplaySettingsA` 探针读回：
  - `WM_COMMAND` + `ID_ROT90/ROT0/ROT270`(1002/1001/1004) → `orient=1 / 0 / 3` ✅
  - `WM_COMMAND` + `ID_CYCLE`(1005) → 方向不变、无报错 ✅
  - `WM_TRAYICON`(0x8001) + `WM_LBUTTONDBLCLK`(0x0203) → 270°→0°→270° ✅
  复用要点：`uID = 1`，且 WndProc **不校验** `wParam`（所以随便填都能进分支）。
- **仍未验收（只剩观感与人眼判断）**：托盘图标在 175%(28px) / 100%(16px) 下的实际清晰度、弹出菜单里勾选项的**渲染**（勾选值本身已由上面的探针证明读得对）、CCD 拔插那一下的黑屏时长，以及**绝对定位输入是否真的跟着恢复** —— 本机唯一的绝对坐标设备是内置屏触摸，没有任何 digitizer 绑定到 `\\.\DISPLAY5`，所以这一条在有触摸/笔的那块屏上只能人眼确认。
...
| 2026-09-18 | 托盘命令路径实机验收（不点鼠标，发窗口消息） | `WM_COMMAND` 四个方向 id + `ID_CYCLE` + `WM_TRAYICON`/双击 全部通过；每步用独立探针读回 |
